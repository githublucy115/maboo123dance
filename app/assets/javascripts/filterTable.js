$(document).ready(function(){
	...
	$('table').each(function(){
		table = $(this);
		if(table.hasClass('filterable')) {
			form = $('<form class="filter"></form>').insertBefore(table)
			input = $('<input type="text" placeholder="Search dancers" />').appendTo(form)
			input.keyup(function(){
				filterTable(input[0],table[0]);
			});
		}
	});
});